...
//메뉴 카테고리(클래식, 프레쉬&라이트, 프리미엄, 아침메뉴)
//items == 각 아이템 name, image, description(문자열 배열)
...
export default function App() {
  // Slide
  const sliderRef = useRef(null); //useRef-초기값 또는 어디 접근할 때 필요
  const sliderSetting = {
      ...
      <section id="section01">
        <div className="slide">
          <Slider ref={sliderRef} {...sliderSetting}>
            {/* <Slide> react-slick slider 컴포넌트 */}
            {/* ref={sliderRef} 슬라이드 인스턴스를 ref로 참조(prev/next 제어 시 필요) */}
            {/* {...sliderSetting} 모든 슬라이더에 설정 전달 */}
            {heroSlides.map((slideClass, index) => (
              <div key={slideClass}>
                <a
                  href="#"
                  className={`slide-item ${slideClass}`}
                  aria-label={`주요 프로모션 ${index + 1}`} //스크린리더를 위한 접근성용 레이블
                ></a>
              </div>
              //className : slide-item i_01, slide-item i_02... 들어감
            ))}
          </Slider>
        </div>
      </section>
            ...
            <ul className="mt_right">
              {menuCategories.map((category, index) => (
                <li
                  ...
                  } ${activeMenuIndex === index ? "on" : ""}`.trim()} //on주면 해당메뉴만 보이게.공백제거
                >
                  <a
                    href="#"
                    onClick={(event) => {
                      event.preventDefault(); //링크금지
                      setActiveMenuIndex(index); //클릭한 거의 인덱스번호를 가져옴
