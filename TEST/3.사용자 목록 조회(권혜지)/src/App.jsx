...
export default function App() {
  ...
  const [filtered, setFiltered] = useState([]); //필터링된 데이터를 저장할 상태
  ...
  const handleSearchChange = (e) => {
    ...
    const filtered = users.filter((item) => {
      return item.name.toLowerCase().includes(searchText.toLowerCase()); // 예시: 데이터 객체의 'name' 속성에 검색어가 포함되어 있는 경우 필터링
    });
